...
3. Many corporate environments use MTIM based TLS inspection, which issues cloned certificates from a corporate trusted CA. While the naming information would match, the certificate’s public key would not. This could create an unintended denial of service on the application.
...
While CT Logs are a Merkle tree that is not simply searchable, there are many sites, like [crt.sh](https://crt.sh/), that gather CT log data, as well as test other aspects of the related PKI infrastructure, and make data more searchable.
